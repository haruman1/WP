<?php
class jupiter_popular_posts extends WP_Widget {
    public function __construct() {
        parent::__construct(
            // Base ID of your widget
            'jupiter_popular_posts', 
            // Widget name will appear in UI
            esc_html__('jupiter Popular Posts', 'jupitercore'),
            // Widget description
            array( 'description' => esc_html__( 'jupiter Popular posts widget.', 'jupitercore' ), ) 
        );
    }    
    ...
    <div class="popular-posts">
        <?php            
                $post_q = new WP_Query(array(
                    ...
                    'meta_key' => 'jupiter_post_views_count',
            ...
            <div <?php post_class( 'post-item'); ?> >
                <?php if(has_post_thumbnail() and $show_thumb == 'on'): ?>
                    <a href="<?php the_permalink(); ?>" class="post-pic">
                        <?php the_post_thumbnail('jupiter_tall_thumbnail'); ?>
                    </a>
                    <?php endif; ?>
                               ...
                               <h4 class="title"><a href="<?php the_permalink(); ?>" rel="bookmark"><?php echo wp_trim_words( get_the_title(), $title_length, '...' ); ?></a></h4>
                                <?php if( $show_meta == 'on' ): ?>
                                    <ul class="post-meta-list">
                                        <!-- Post-Meta-Item -->
                                        <li class="post-meta-item post-comment-count"> <span class="icon"><i class="far fa-calendar-alt"></i></span> <span class="value"><?php echo get_the_date(); ?></span> </li>
                                    </ul>
                                    <?php endif; ?>
                        </div>
            </div>
        ...
        ?>
            <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>">
                    <?php esc_html_e( 'Title:','jupitercore' ); ?>
                </label>
                <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /> 
            </p>
            <p>
                <label for="<?php echo $this->get_field_id( 'title_length' ); ?>">
                    <?php esc_html_e('Post Title Length:','jupitercore'); ?>
                </label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'number' ); ?>">
                    <?php esc_html_e('Number of posts to show:','jupitercore'); ?>
                </label>
            ...
            <p>
                ...
                <label for="<?php echo $this->get_field_id( 'show_meta' ); ?>">
                    <?php esc_html_e('Display Post Meta?','jupitercore'); ?>
                </label>
            </p>
            <p>
                ...
                <label for="<?php echo $this->get_field_id( 'show_thumb' ); ?>">
                    <?php esc_html_e('Display Post Thumbnail?','jupitercore'); ?>
                </label>
            </p>
            <?php
    }    
