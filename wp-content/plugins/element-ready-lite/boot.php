...
final class Element_Ready_Elementor_Extension {
    ...
    */
	...
	public function admin_notice_missing_main_plugin() {
			...
			$product_name = esc_html__('Element Ready Lite','element-ready');
			$con = esc_html__( 'Click to Install', 'shop-ready');
		
			if( file_exists(WP_PLUGIN_DIR .'/elementor/elementor.php' ) ) {
		
				$er_url = $this->plugin_activation_link_url('elementor/elementor.php');
				$con = esc_html__( 'Click to Activate', 'shop-ready');
			...
			if ( in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
				...
				$message = sprintf(
					esc_html__( '"%1$s" requires "%2$s" %3$s', 'element-ready' ),
					'<strong>' . $product_name . '</strong>',
					'<strong>' . esc_html__( 'Elementor', 'element-ready' ) . '</strong>',
			...
			printf( '<div class="notice shop-ready-notice notice-warning is-dismissible"><p>%1$s</p></div>', $message );
