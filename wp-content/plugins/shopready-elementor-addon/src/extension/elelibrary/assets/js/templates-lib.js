...
;( function( elementor, $, window ) {
	...
	var QSTemplateLibraryTemplateModel = Backbone.Model.extend({
		defaults: {
			template_id: 0,
			title: '',
			source: '',
	...
	var TemplateLibraryCollection = Backbone.Collection.extend({
		model: QSTemplateLibraryTemplateModel
	});

	var QSTemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		ui: {
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('wooreadysrc/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateView = Marionette.ItemView.extend({
		...
		behaviors: {
			insertTemplate: {
				behaviorClass: QSTemplateLibraryInsertTemplateBehavior
			}
		}
	});

	var QSTemplateLibraryTemplateView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-shop-ready',
		ui: function ui() {
		...
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			shop_ready_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && shop_ready_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-shop-ready',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'wooreadysrc' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-shop-ready',
		...
		ui: {
			...
			clickFilter: '.woo-ready-template-library-cat-text',
		...
		getChildView: function getChildView(childModel) {
			return QSTemplateLibraryTemplateView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = shop_ready_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = shop_ready_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : shop_ready_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(shop_ready_templates_lib.templates.getFilter('text') || shop_ready_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('wooreadysrc' === shop_ready_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('wooreadysrc' === shop_ready_templates_lib.templates.getFilter('source') && 'page' !== shop_ready_templates_lib.templates.getFilter('type')) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			shop_ready_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onTextFilterClick: function onTextFilterClick(event) {
			...
			shop_ready_templates_lib.templates.setFilter(filterName,text);
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
				
			shop_ready_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			shop_ready_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo_shop-ready',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-shop-ready',
		...
		onSyncClick: function() {
			...
			shop_ready_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('wooreadysrc');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('wooreadysrc').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getLogoOptions: function() {
			return {
				title: 'Shop Ready Templates',
				click: function click() {
					$e.run('wooreadysrc/open', {
						toDefault: true
					});
					return false;
				}
		...
		getTemplateActionButton: function(templateData) {
			...
			if ( templateData.is_pro ) {
				viewId = '#tmpl-elementor-template-library-get-pro-button-shop-ready';
			}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.activateTab = function(tab) {
			$e.routes.saveState('wooreadysrc');
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'wooreadysrc' ) ) {
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('woo_ready_get_library_data', ajaxOptions);
		};
		...
		this.filterTemplates = function () {
			...
			return templatesCollection.filter(function (model) {
				if ('wooreadysrc' !== model.get('source')) {
					return false;
				}
	...
	shop_ready_templates_lib.templates = new TemplateLibraryManager();
	shop_ready_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.elementor-add-section-area-button.woo-ready-add-template-button { margin-left: 8px; vertical-align: bottom; }';
			btnStyle += '.elementor-add-section-area-button.woo-ready-add-template-button img { height: 40px; }';
			btnStyle += '.elementor-add-new-section .elementor-add-section-area-button.woo-ready-add-template-button:hover { -webkit-box-shadow: none; box-shadow:none; }';
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button woo-ready-add-template-button' );
		$templateLibBtn.attr( 'title', 'Add Shop Ready Template' );
		$templateLibBtn.html( '<img src="' + shop_ready_templates_lib.logoUrl + '" />' );
		...
		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', function(e) {
			...
			setTimeout(function() {
				...
				if ( sectionInner.length > 0 ) {
					sectionInner.find( '.woo-ready-add-template-button' ).remove();
