...
class Templates_Lib {
	...
	public static function enqueue_editor_scripts() {
        ...
        wp_enqueue_style( 'woo-ready-templates-lib', SHOP_READY_TEMPLATE_MODULE_URL . 'assets/css/editor.css',1.1,true );
		wp_enqueue_script(
			'woo-ready-templates-lib',
		...
		wp_localize_script( 'woo-ready-templates-lib', 'shop_ready_templates_lib', array(
			'logoUrl'	=> SHOP_READY_PUBLIC_ROOT_IMG.'template-icon.svg',
		) );
	}
	 ...
	 */
	public static function woo_ready_get_library_data( array $args ) {
	...
	public static function get_templates() {
		$source = Plugin::$instance->templates_manager->get_source( 'wooreadysrc' );
	...
	private static function get_templates_data( $force_update = false ) {
		...
		$cache_key = self::LIBRARY_OPTION_KEY.'_data_' . 1.2;
	...
	public static function get_template_content( $template_id ) {
		...
		$_key = apply_filters( 'woo_ready_product', '*' );
	...
	public static function render_template() {
		?>
		<script type="text/template" id="tmpl-elementor-template-library-header-actions-shop-ready">
			<div id="elementor-template-library-header-sync" class="elementor-templates-modal__header__item">
				<i class="eicon-sync" aria-hidden="true" title="<?php esc_attr_e( 'Sync Templates', 'shopready-elementor-addon' ); ?>"></i>
				<span class="elementor-screen-only"><?php echo esc_html__( 'Sync Templates', 'shopready-elementor-addon' ); ?></span>
			</div>
		</script>
		<script type="text/template" id="tmpl-elementor-templates-modal__header__logo_shop-ready">
			<span class="elementor-templates-modal__header__logo__icon-wrapper">
				<img src="<?php echo esc_url( SHOP_READY_PUBLIC_ROOT_IMG.'logo.svg' ); ?>" style="height: 30px;" />
			</span>
			<span class="elementor-templates-modal__header__logo__title">{{{ title }}}</span>
		</script>
		<script type="text/template" id="tmpl-elementor-template-library-header-preview-shop-ready">
			<div id="elementor-template-library-header-preview-insert-wrapper" class="elementor-templates-modal__header__item">
				{{{ shop_ready_templates_lib.templates.layout.getTemplateActionButton( obj ) }}}
			</div>
			
		</script>
		<script type="text/template" id="tmpl-elementor-template-library-templates-shop-ready">
			<#
				var activeSource = shop_ready_templates_lib.templates.getFilter('source');
			#>
			<div id="elementor-template-library-toolbar">
				<# if ( 'wooreadysrc' === activeSource ) {
					var activeType = shop_ready_templates_lib.templates.getFilter('type');
					var config = shop_ready_templates_lib.templates.getConfig( activeType );
					#>
					<div id="elementor-template-library-filter-toolbar-remote" class="elementor-template-library-filter-toolbar">
							...
							<div id="elementor-template-library-order">
								...
								<label for="elementor-template-library-order-new" class="elementor-template-library-order-label"><?php echo esc_html__( 'New', 'shopready-elementor-addon' ); ?></label>
								...
								<label for="elementor-template-library-order-trend" class="elementor-template-library-order-label"><?php echo esc_html__( 'Trend', 'shopready-elementor-addon' ); ?></label>
								...
								<label for="elementor-template-library-order-popular" class="elementor-template-library-order-label"><?php echo esc_html__( 'Popular', 'shopready-elementor-addon' ); ?></label>
							</div>
							
						
						<div id="elementor-template-library-my-favorites">
							<# var checked = shop_ready_templates_lib.templates.getFilter( 'favorite' ) ? ' checked' : ''; #>
							...
							<label id="elementor-template-library-filter-my-favorites-label" for="elementor-template-library-filter-my-favorites">
								...
								<?php echo esc_html__( 'My Favorites', 'shopready-elementor-addon' ); ?>
							</label>
						</div>
					</div>
				<# } #>
				<div id="elementor-template-library-filter-text-wrapper">
					<label for="elementor-template-library-filter-text" class="elementor-screen-only"><?php echo esc_html__( 'Search Templates:', 'shopready-elementor-addon' ); ?></label>
					<input id="elementor-template-library-filter-text" placeholder="<?php echo esc_attr__( 'Search', 'shopready-elementor-addon' ); ?>">
			...
			<div class="elementor-template-library-templates-container-wrapper">
				<# if ( config.categories ) { #>
					<div id="elementor-template-library-toolbar-cats">
						<div class="woo-ready-template-popup">
							<# config.categories.forEach( function( category ) {
								var selected = category === shop_ready_templates_lib.templates.getFilter( 'subtype' ) ? ' selected' : '';
								#>

								<label data-elementor-text="{{ category }}" data-elementor-filter="subtype" class="woo-ready-template-library-cat-text {{ selected }}">{{{ category }}}</label>
			...
			<# if ( 'wooreadysrc' === activeSource ) { #>
				<div id="elementor-template-library-footer-banner">
					...
					<div class="elementor-excerpt"><?php echo esc_html__( 'Stay tuned! More awesome templates coming real soon.', 'shopready-elementor-addon' ); ?></div>
				</div>
			<# } #>
		</script>
		<script type="text/template" id="tmpl-elementor-template-library-template-shop-ready">
			...
			<div class="elementor-template-library-template-footer">
				...
				<div class="elementor-template-library-favorite">
					...
					<label for="elementor-template-library-template-{{ template_id }}-favorite-input" class="elementor-template-library-template-favorite-label">
						...
						<span class="elementor-screen-only"><?php echo esc_html__( 'Favorite', 'shopready-elementor-addon' ); ?></span>
					</label>
				</div>
			</div>
		</script>
		<script type="text/template" id="tmpl-elementor-template-library-get-pro-button-shop-ready">
			<a class="elementor-template-library-template-action elementor-button elementor-go-pro" href="<?php echo esc_url(SHOP_READY_DEMO_URL); ?>" target="_blank">
				<i class="eicon-external-link-square" aria-hidden="true"></i>
			
				<span class="elementor-button-title"><?php echo esc_html__( 'Go Pro', 'shopready-elementor-addon' ); ?></span>
			</a>
		</script>
		<script type="text/template" id="tmpl-elementor-pro-template-library-activate-license-button-shop-ready">
			<a class="elementor-template-library-template-action elementor-button elementor-go-pro" href="<?php echo esc_url(SHOP_READY_DEMO_URL); ?>" target="_blank">
				<i class="eicon-external-link-square"></i>
				<span class="elementor-button-title"><?php echo esc_html__( 'Activate License', 'shopready-elementor-addon' ); ?></span>
			</a>
		</script>
		<?php
	}
