	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'show_desc',
				[
					'label'        => esc_html__( 'Show Desc ?', 'shopready-elementor-addon' ),
					'type'         => Controls_Manager::SWITCHER,
					'label_on'     => esc_html__( 'Show', 'shopready-elementor-addon' ),
					'label_off'    => esc_html__( 'Hide', 'shopready-elementor-addon' ),
					'return_value' => 'yes',
		...
		$this->start_controls_section(
			...
			$this->start_controls_tabs( 'title_tab_style' );
				$this->start_controls_tab(
					...
					$this->add_control(
						'title_text_value_color',
						[
							'label'     => esc_html__( 'Value Color', 'shopready-elementor-addon' ),
							'type'      => Controls_Manager::COLOR,
							'default'   => '',
							'selectors' => [
								'{{WRAPPER}} .area__title span' => 'color: {{VALUE}};',
							],
						]
					);
					$this->add_group_control(
		...
		$this->text_minimum_css(
            [
                ...
                'slug'           => '_popularity_desc',
                'element_name'   => 's__popularity_box',
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .area__content' => 'position: {{VALUE}};',
					],
				]
			);
		$this->end_controls_section();
	...
	protected function html() {
		...
		if ( 'yes' == $settings['show_icon'] ) {
			if ( 'font_icon' == $settings['icon_type'] && !empty( $settings['font_icon'] ) ) {
				...
				$icon       = '<div class="area__icon '. esc_attr( $icon_animation ) .'">'. wp_kses_post($sr_img) .'</div>';
		...
		if ( $settings['show_desc'] =='yes' ) {
			$description = '<div class="area__description">'.wpautop( get_the_archive_description() ).'</div>';
		}else{
			$description = '';
		}
		...
		echo'<div class="area__content">'; ?>
			<?php if ( 'yes' == $settings['show_bg_icon'] ) :  ?>
				<?php if ( 'font_icon' == $settings['bg_icon_type'] && !empty($settings['bg_font_or_svg']) ) : ?>
					<div class="title__bg__icon"><?php echo wp_kses_post(shop_ready_render_icons( $settings['bg_font_or_svg'] )); ?></div>
