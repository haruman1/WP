...
<section class="woocommerce-order-details woo-ready-order-details">
    ...
    <?php if(shop_ready_gl_get_setting('woo_ready_enable_thankyou_order_details_heading','yes') =='yes'): ?>
		<h2 class="woocommerce-order-details__title"><?php echo shop_ready_gl_get_setting('woo_ready_thankyou_order_details_heading','Order Details'); ?></h2>
	<?php endif; ?>
	...
	<table class="woocommerce-table woocommerce-table--order-details shop_table order_details">
		...
		<thead>
			<tr>
				<th class="woocommerce-table__product-name product-name"><?php esc_html_e( 'Product', 'shopready-elementor-addon' ); ?></th>
				<th class="woocommerce-table__product-table product-total"><?php esc_html_e( 'Total', 'shopready-elementor-addon' ); ?></th>
			</tr>
		</thead>
			...
			foreach ( $order->get_order_item_totals() as $key => $total ) {
				?>
					<tr>
						...
						<td> <?php echo wp_kses_post( 'payment_method' === $key  ? esc_html( $total['value'] ) :  $total['value'] ); ?></td>
					</tr>
					<?php
			}
			?>
