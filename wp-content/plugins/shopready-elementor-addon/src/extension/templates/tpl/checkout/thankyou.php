...
<div class="woocommerce-order woo-ready-thanks-orders">
	...
	if ( $order ) :
		...
		<?php if ( $order->has_status( 'failed' ) ) : ?>
			...
			?>
			<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed"><?php echo shop_ready_gl_get_setting('woo_ready_thank_you_order_fail_msg') == ''? esc_html__( 'Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'shopready-elementor-addon' ): shop_ready_gl_get_setting('woo_ready_thank_you_order_fail_msg'); ?></p>

			<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed-actions">
				<a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" class="button pay"><?php echo esc_html(shop_ready_gl_get_setting('woo_ready_thank_you_order_pay_text') == ''? esc_html__( 'Pay', 'shopready-elementor-addon' ) : shop_ready_gl_get_setting('woo_ready_thank_you_order_pay_text')); ?></a>
				<?php if ( shop_ready_gl_get_setting('woo_ready_enable_thankyou_fail_myaccount','yes') == 'yes' && is_user_logged_in() ) : ?>
					<a href="<?php echo esc_url( $fail_default ); ?>" class="button pay"><?php echo esc_html(shop_ready_gl_get_setting('woo_ready_thank_you_order_fail_myaccount_text') == ''? esc_html__( 'My Account', 'shopready-elementor-addon' ) : shop_ready_gl_get_setting('woo_ready_thank_you_order_fail_myaccount_text')); ?></a>
				<?php endif; ?>
			</p>
		...
		<?php else : ?>
            ...
            <?php if(shop_ready_gl_get_setting('woo_ready_enable_thankyou_msg','yes') == 'yes'): ?>
				<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received"><?php echo shop_ready_gl_get_setting('woo_ready_thank_you_msg','Thank you. Your order has been received.'); ?></p>
            <?php endif; ?>
			...
			<ul class="woocommerce-order-overview woocommerce-thankyou-order-details order_details">
				...
				<?php if ( shop_ready_gl_get_setting('woo_ready_enable_thankyou_email','yes') == 'yes' && is_user_logged_in() && $order->get_user_id() === get_current_user_id() && $order->get_billing_email() ) : ?>
					...
					<?php echo esc_html(shop_ready_gl_get_setting('woo_ready_thank_you_order_email','Email:')); ?>
						<strong><?php echo sanitize_email($order->get_billing_email()); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
					</li>
				<?php endif; ?>
	...
	<?php else : ?>
		...
		<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received"><?php echo esc_html(shop_ready_gl_get_setting('woo_ready_thank_you_msg','Thank you. Your order has been received.')); ?></p>
