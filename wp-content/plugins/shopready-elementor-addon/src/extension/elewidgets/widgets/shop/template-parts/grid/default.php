            ...
            <div class="wooready_product_cart_box display:flex justify-content:flex-start align-items:center position:absolute">
                ...
                <div class="wooready_product_cart">
                    <?php if($product->is_type('simple')): ?> 
                        <a class="ajax_add_to_cart <?php echo esc_attr($product->get_type()); ?>" data-product-type='<?php echo esc_attr($product->get_type()); ?>' data-quantity="1" data-product_id="<?php echo esc_attr($product->get_id()); ?>" href="<?php echo esc_url($addToCartUrl); ?>"><?php echo wp_kses_post($settings['add_to_cart_icon']); ?></i> <?php echo wp_kses_post($settings['cart_text']); ?></a>
                        <?php else: ?>
                        <a class="add-to-cart <?php echo esc_attr($product->get_type()); ?>" href="<?php echo esc_url( $addToCartUrl ); ?>"><?php echo wp_kses_post( $settings['add_to_cart_icon'] ); ?></i> <?php echo esc_html( $settings['cart_text'] ); ?></a>
                    <?php endif; ?>
                </div>
