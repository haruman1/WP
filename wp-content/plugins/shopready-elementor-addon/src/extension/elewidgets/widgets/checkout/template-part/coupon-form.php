...
<?php if( $settings['wready_coupon_collapsible'] =='yes' ): ?>
	<div class="woocommerce-form-coupon-toggle">
		<?php wc_print_notice( apply_filters( 'woocommerce_checkout_coupon_message', '<span>'.esc_html( $settings['coupon_heading_colapse_msg_text'] ).'</span>' . ' <a href="#" class="showcoupon">' . esc_html($settings['coupon_heading_colapse_text']) . '</a>' ), 'notice' ); ?>
	</div>
<?php endif; ?>
...
<form class="checkout_coupon woocommerce-form-coupon" method="post" <?php echo wp_kses_post($css_style); ?>>
    <div class="wready-coupon-row-wrapper">
		...
		<p class="form-row form-row-first woo-ready-coupon-col">
			<input type="text" name="coupon_code" class="input-text" placeholder="<?php echo esc_attr($apply_code_label); ?>" id="coupon_code" value="" />
		</p>

		<p class="form-row form-row-last wready-coup-btn-col">
			<button type="submit" class="button" name="apply_coupon" value="<?php echo esc_attr($apply_coupon_label); ?>"><?php echo esc_html($apply_coupon_label); ?></button>
		</p>
	</div>
</form>