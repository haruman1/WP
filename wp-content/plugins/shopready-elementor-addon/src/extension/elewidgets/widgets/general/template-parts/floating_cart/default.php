...
?>
    ...
    <div class="woo-ready-cart-box">
        ...
        <?php elseif ( 'template' == $settings['floating_cart_content_type'] ) : ?>
            <?php if ( !empty( $settings[ 'floating_cart_content_template'] ) ) {
                echo '<div class="shopping_cart_template_content">';
                    echo \Elementor\Plugin::$instance->frontend->get_builder_content( $settings[ 'floating_cart_content_template' ], true );
                echo '</div>';
            } ?>
        <?php endif; ?>
    </div>