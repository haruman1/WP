<div <?php post_class('post-single'); ?> >   
    <?php    
        ...
        if( $jupi_opt['is_blog_post_image'] == true ){
            if(get_post_meta( $id, '_jupi_post_video_embed', true )){
                $video_url = get_post_meta( $id, '_jupi_post_video_embed', true );
            }else{
                $video_url = '';
            }
            if( !empty($video_url) && function_exists('jupi_video_embed_content') ):
                // Video Post Content Function
                echo jupi_video_embed_content(esc_url($video_url));
            else:   
