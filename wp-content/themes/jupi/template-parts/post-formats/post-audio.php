<div <?php post_class('post-single'); ?> >   
    <?php    
        ...
        if( $jupi_opt['is_blog_post_image'] == true ){
            if(get_post_meta( $id, '_jupi_post_audio_embed', true )){
                $audio_url = get_post_meta( $id, '_jupi_post_audio_embed', true );
            }else{
                $audio_url = '';
            }
            if( !empty($audio_url) && function_exists('jupi_audio_embed_content') ):
                // Audio Post Content Function
                echo jupi_audio_embed_content(esc_url($audio_url));
            else:
